"""
Интерактивная тепловая карта потенциала двух линз.

Переключение: точечные массы / SIS.
Слайдеры: theta_E1, theta_E2, расстояние.
Круги Эйнштейна + эквипотенциали.
"""
...
class DualLensPotentialExplorer:

    def __init__(self):
        self.te1 = 1.0
        self.te2 = 1.0
        self.sep = 0.3
        self.model = "Point Mass"  # или "SIS"
    ...
    def _build_figure(self):
        ...
        self.ax = self.fig.add_axes([0.08, 0.22, 0.68, 0.72])
        self.ax.set_aspect("equal")
        self.ax.set_xlabel(r"$\theta_x$", fontsize=13)
        self.ax.set_ylabel(r"$\theta_y$", fontsize=13)

        self.cbar_ax = self.fig.add_axes([0.80, 0.22, 0.03, 0.72])

        self.mesh = None
        self.cbar = None

        # Круги — рисуем как plot (а не patch), чтобы zorder гарантированно работал
        self.circle1_line, = self.ax.plot([], [], "-", color="white", linewidth=2.5, alpha=0.9, zorder=10)
        self.circle2_line, = self.ax.plot([], [], "-", color="white", linewidth=2.5, alpha=0.9, zorder=10)
    ...
    def _draw(self):
        ...
        r1 = np.sqrt((self.X - x1)**2 + self.Y**2)
        r2 = np.sqrt((self.X - x2)**2 + self.Y**2)
        ...
        self.mesh = self.ax.pcolormesh(
            self.X, self.Y, psi_masked, shading="auto", cmap="inferno", zorder=1)
        ...
        if vmax - vmin > 1e-10:
            ...
            self.ax.contour(
                self.X, self.Y, psi_masked, levels=levels,
                colors="white", linewidths=0.5, alpha=0.6, zorder=2)

        # Круги Эйнштейна (через plot — гарантированно сверху)
        ...
        # Colorbar — очищаем оси, а не удаляем
        self.cbar_ax.clear()
        self.cbar = self.fig.colorbar(self.mesh, cax=self.cbar_ax)
        self.cbar.set_label(r"$\psi(\theta)$", fontsize=12)

    def show(self):
        plt.show()


explorer = DualLensPotentialExplorer()
explorer.show()
